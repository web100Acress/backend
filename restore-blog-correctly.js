...
async function restoreBlogCorrectly() {
  try {
    // Connect to database
    ...
    // Update the blog using the correct field name: blog_Description
    const updatedBlog = await blogModel.findByIdAndUpdate(
      blogId,
      {
        blog_Description: blogContent, // Use blog_Description instead of blog_Content
    ...
    if (updatedBlog) {
      ...
      console.log(`   Description Length: ${updatedBlog.blog_Description ? updatedBlog.blog_Description.length : 0}`);
      console.log(`   Published: ${updatedBlog.isPublished ? 'Yes' : 'No'}`);
      console.log(`   Updated: ${updatedBlog.updatedAt}`);
      
      if (updatedBlog.blog_Description) {
        console.log('\n📝 First 200 characters of content:');
        console.log(updatedBlog.blog_Description.substring(0, 200) + '...');
      }
    } else {
